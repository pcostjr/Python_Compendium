...
class Dice:
    ...
    # return the value of a die at index
    ...
    def score(self):
        ...
        if self.of_a_kind(5):
            ...
            print("Full House!")
        elif self.straight(0, 4) or self.straight(1, 5):
            print("Straight!")
    ...
    def straight(self, start, end):
        ...
        while result and i <= end:
            if self.dice_faces[i] != 1:
                print(self.dice_faces[i])
                result = False
            i += 1
        return result
    ...
    def __str__(self):
        dice_values = [self.dice[i].get_value() for i in range(self.MAX_DICE)]
        # TODO: add keep value stuff
        # out_string += " " if self.keep[i] else "| Kept "
        return display_dice(dice_values)
